...
## Installing *Miniconda* {#uv-install}

Since *agx-emulsion* relies on Python, the use of *Miniconda* as a Python manager is a nice solution, as it is both light and cross-platform.

To install *Miniconda*, follow the instructions provided on the [Anaconda website](https://www.anaconda.com/docs/getting-started/miniconda/install) for the three main operating systems (Windows, MacOS and Linux). It is best to install Miniconda for the local user, which does not require administrator permissions and is the most robust type of installation. This way *Miniconda* will be installed in a new subdirectory of the active user by default, which is: `C:\%userprofile%\miniconda3` on Windows, `~/miniconda3` on MacOS and Linux.
...
### **Dowloading the *agx-emulsion*** **code** {#agx-emulsion-installation}
...
> You should see `(agx-emulsion)` in the command line prompt.
